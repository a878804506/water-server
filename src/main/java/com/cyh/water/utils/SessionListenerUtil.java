...
import com.cyh.water.utils.zookeeperUtil.ZookeeperUtil;
...
public class SessionListenerUtil implements HttpSessionListener,Runnable {
    ...
    public void sessionCreated(HttpSessionEvent htpSessionEvent) {
        ...
        //nginx 代理用户头像所在服务器
        htpSessionEvent.getSession().setAttribute("staticPictureHost", Properties.ftpStaticPictureHost);
    ...
    public void sessionDestroyed(HttpSessionEvent htpSessionEvent) {
        ...
        if(null != user){
            ...
            //redis 中该用户状态改为离线
            if(Properties.webSocketChatSwitch) { //开启了项目的聊天功能
    ...
    private static void updateUserStrutsToOffLine(User user){
        ...
        try {
            ...
            //再次从redis获取联系人列表
            List<Map<String,Object>> redis_contactsList = SerializeUtil.unserializeForList(jedis.get(Properties.redisSystemUsers.getBytes()));
            ...
            jedis.set(Properties.redisSystemUsers.getBytes(), SerializeUtil.serialize(redis_contactsList));
